...
def train_fn(nEpochs,image_path):
    ...
    for i in range(n):
        ...
        _,curr_loss_d,d_logits = sess.run([d_optim,loss['loss_d'],dcgan.conv4_out],feed_dict={dcgan.z:z,dcgan.real_images:real_imgs,dcgan.train:True})
        ...
        if i%100==0:
            for j in range(5):
                img = (gen_imgs[j] + 1)*127.5
                cv2.imwrite('../Output/' + str(j) + '_' + str(i) + '_Gen.jpg',img)
